...
public class CachePutGetExample {
    static {
        System.loadLibrary("org_apache_ignite_examples_datagrid_CachePutGetExample");
    }
    private native String cppget(IgniteCache<Integer, String> cache, Integer index);
    /** Cache name. */
    ...
    private static void putGet(IgniteCache<Integer, String> cache) throws IgniteException {
        ...
        for (int i = 0; i < keyCnt; i++) {
            String cppresult = new CachePutGetExample().cppget(cache, i);
            System.out.println("Cppresult: Got [key=" + i + ", val=" + cppresult + ']');
        }
    }
    /**
