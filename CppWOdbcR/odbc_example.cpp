...
void QueryDataFJava(SQLHDBC dbc)
{
    std::cout << std::endl;
    std::cout << ">>> Getting list of city From Java:" << std::endl;

    GetDataWithOdbc(dbc, "SELECT p.name, c.name FROM Public.Personj p INNER JOIN Public.City c on c.id = p.city_id");
}

void QueryDataFCpp(SQLHDBC dbc)
{
    std::cout << std::endl;
    std::cout << ">>> Getting list of Org From Cpp:" << std::endl;

    GetDataWithOdbc(dbc, "SELECT o.name FROM myCache.Orgnization o");
}
...
int main()
{
    ...
    try
    {
        ...
        //cache::Cache<int, examples::Organization> cache = grid.GetOrCreateCache<int, examples::Organization>("MYCACHE");
        ...
        // Populate caches.
        //PopulatePerson(dbc);
        cache::Cache<int64_t, Person> cache = grid.GetCache<int64_t,Person>("Person");
        //Person p = cache.Get((int64_t)1);
        //PopulateOrganization(dbc);
        Person p(1, "Joe", "Doe", "Master Degree", 2200.00);
        cache.Clear();
        //make sure cache is cleared and empty
        std::cout << cache.Size();
        cache.Put(2, p);
